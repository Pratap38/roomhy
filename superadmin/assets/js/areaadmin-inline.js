...
const sidebarConfig = {
    ...
    'kyc_verification': { label: 'KYC Verification', page: 'kyc_verification.html', icon: 'file-check' },
    ...
    'reports': { label: 'Reports', page: 'platform_reports.html', icon: 'bar-chart-2' },
    'profile': { label: 'Profile', page: 'profile.html', icon: 'user' },
    'settings': { label: 'Settings', page: 'settings.html', icon: 'settings' }
};
...
function loadPage(pageUrl, element) {
    ...
    if (pageUrl === 'dashboard_home') {
        ...
        const resolvedUrl = isReactAreaAdmin && pageUrl.endsWith('.html')
            ? `/superadmin/${pageUrl.replace(/\.html$/i, '')}?embed=1`
            : pageUrl;
...
function initAreaAdmin() {
    ...
    const sys = ['web_enquiry', 'enquiries', 'bookings', 'reviews', 'complaint_history', 'kyc_verification', 'live_properties', 'locations', 'reports'];
