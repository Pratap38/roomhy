lucide.createIcons();
        ...
        function readVisitsFromStorage() {
            ...
            const mergeById = (a, b) => {
                const map = new Map();
                (a || []).forEach(v => { if (v && (v._id || v.visitId)) map.set(v._id || v.visitId, v); });
                (b || []).forEach(v => { if (v && (v._id || v.visitId)) map.set(v._id || v.visitId, v); });
                return Array.from(map.values());
            };
            try {
                if (window.safeStorage && typeof window.safeStorage.getVisits === 'function') {
                    const safe = window.safeStorage.getVisits();
                    if (Array.isArray(safe) && safe.length) return safe;
                }
            } catch (_) {}
            let local = [];
            let session = [];
            try { local = JSON.parse(localStorage.getItem('roomhy_visits') || '[]'); } catch(_) { local = []; }
            try { session = JSON.parse(sessionStorage.getItem('roomhy_visits') || '[]'); } catch(_) { session = []; }
            visits = mergeById(local, session);
            try {
                if (visits.length === 0 && window.__roomhy_memory && Array.isArray(window.__roomhy_memory.visits)) {
                    visits = window.__roomhy_memory.visits.slice();
                }
            } catch (_) {}
            return visits;
        }
        ...
        async function fetchEnquiries() {
            ...
            tbody.innerHTML = visits.map((v, idx) => {
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-4 py-3">
                        <button onclick="openMap('${v.visitId}')" class="text-xs px-2 py-1 bg-gray-50 rounded border text-gray-600 hover:bg-gray-100" ${!(v.latitude && v.longitude) ? 'disabled' : ''}>Map</button>
                    </td>
                    ...
                    <td class="px-4 py-3 text-center">
                        <div class="flex items-center justify-center gap-2">
                            <button onclick="approve('${v.visitId}')" class="p-2 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors" title="Approve"><i data-lucide="check" class="w-4 h-4"></i></button>
                            <button onclick="holdVisit('${v.visitId}')" class="p-2 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200 transition-colors" title="Hold"><i data-lucide="pause" class="w-4 h-4"></i></button>
                        </div>
                    </td>
                </tr>`;
            }).join('');
