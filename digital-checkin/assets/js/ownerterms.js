    ...
    async function loadOwnerDetails() {
      ...
      try {
        ...
        document.getElementById('editProfileBtn').onclick = () => {
          ...
          location.href = `ownerprofile.html?loginId=${encodeURIComponent(loginId)}${emailPart}${areaPart}`;
        };

        document.getElementById('editKycBtn').onclick = () => {
          const emailPart = email ? `&email=${encodeURIComponent(email)}` : '';
          location.href = `ownerkyc.html?loginId=${encodeURIComponent(loginId)}${emailPart}`;
        };
      } catch (err) {
