lucide.createIcons();
        // Header user info logic (copied from admin.html)
            ...
            try {
                ...
                const rows = list.map(enquiry => {
                    ...
                    if (enquiry.status === 'accepted' && enquiry.chatOpen) {
                        actionBtn = `<a href="chat.html?enquiryId=${enquiry._id}&studentId=${enquiry.studentId}" class="ml-2 bg-blue-600 text-white px-2 py-1 rounded text-xs">Chat</a>`;
                    }
